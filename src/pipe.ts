...
type Flow<
  ...
  ReducedFn extends UnaryFunction
> = Fns["length"] extends 0
  ? ReducedFn
  : Head<Fns> extends UnaryFunction<
      infer A extends ReturnType<ReducedFn>,
      infer R
    >
...
type CombineTest = Combine<
  [(x: string) => boolean, (x: string) => number, (x: Date) => string]
>;
...
export type Pipeline<Functions extends UnaryFunction[]> =
  Functions["length"] extends 1
    ...
    ? [
        ...
        ...Pipeline<
          [
            FirstRes extends SecondArg
              ? UnaryFunction<SecondArg, SecondRes>
              : never,
            ...Rest
          ]
        >
...
type Composition<Functions extends UnaryFunction[]> = Reverse<
  Pipeline<Reverse<Functions>>
>;
type CompositionTest = Composition<
  [(x: number) => boolean, (x: string) => number, (x: Date) => string]
>;
